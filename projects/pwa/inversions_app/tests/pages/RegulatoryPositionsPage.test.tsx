...
vi.mock("../../src/services/institutional/institutionalApi", async () => {
  const actual = await vi.importActual<typeof import("../../src/services/institutional/institutionalApi")>(
    "../../src/services/institutional/institutionalApi"
  );
  return {
    ...actual,
    getRegulatoryPositions: vi.fn(async () => ({
      ...
      positions13F: [
        { sourceId: "sec", asOf: "2026-05-20", count: 150, notional: 3e7, fundsOwnershipPct: 6, volume: 1e5, confidence: 0.9 }
      ],
      flows: { inflows: 2e6, outflows: 1e6, netFlow: 1e6, asOf: "2026-05-20" },
      sourceReports: [
        { sourceId: "src-1", kind: "13F", label: "SEC Filing", status: "ok" as const, latencyMs: 100 }
      ],
      cacheHit: true,
      usedSourceIds: ["sec"]
    }))
