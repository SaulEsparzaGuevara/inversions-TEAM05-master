# Data Model: 009-team-05-institucional-migration
...
## Coverage Engine Output Fields

### CollarEngine RiskMetrics

| Campo | Tipo | Fórmula | Nota |
|-------|------|---------|------|
| `maxProfit` | `number` | `max(0, callStrike - currentPrice - netPremiumPerShare) * shares` | Corregido en spec 008 |
| `maxLoss` | `number` | `max(0, currentPrice - putStrike + netPremiumPerShare) * shares` | Correcto desde inicio |
| `protectionCeilingPrice` | `number` | `callStrike - netPremiumPerShare` | Corregido en spec 008 |
| `protectionFloorPrice` | `number` | `putStrike - netPremiumPerShare` | Correcto desde inicio |
| `exerciseRiskScore` | `number` [0,1] | `clamp01(downside*0.5 + upside*0.5)` | Corregido en spec 008 |
| `netPremiumPerShare` | `number` | `putPremium - callPremium` | Positivo=débito, negativo=crédito |

### ProtectivePutEngine RiskMetrics

| Campo | Tipo | Fórmula | Nota |
|-------|------|---------|------|
| `breakEvenPrice` | `number` | `currentPrice + netPremiumPerShare` | Corregido en spec 008 |
| `stopLossPrice` | `number` | `putStrike * (1 - clamp(riskTolerancePct*0.5, 0.01, 0.10))` | Corregido en spec 008 |
| `maxProfit` | `number` | `Infinity` | Ilimitado al alza |
| `maxLoss` | `number` | `max(0, currentPrice - putStrike + netPremiumPerShare) * shares` | Correcto desde inicio |
...
### Merge Strategy Data Flow

```
Source A ──┐
Source B ──┤── Promise.allSettled ──► Normalize ──► Merge ──► overallStatus
Source C ──┤                                    │
Source D ──┘                                    ├─ fundsOwnershipPct → PROMEDIO
                                                 ├─ volume → MÁXIMO
                                                 ├─ flows.inflows/outflows → SUMA
                                                 ├─ openPositions.count → MÁXIMO
                                                 ├─ categorical → FIRST DEFINED
                                                 └─ liquidity → HIGHEST
```
